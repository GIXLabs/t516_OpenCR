...
static const TB3ModelInfo gix_burger_info = {
  "GIX_Burger",
  4,
  0.033,
  0.160,
...
static const TB3ModelInfo gix_waffle_info = {
  "GIX_Waffle",
  5,
  0.033,
  0.287,
...
static GIXMotorDriver gix_motor_driver(motor_driver.getDxl());
...
static bool get_connection_state_with_gix();
static void set_connection_state_with_gix(bool is_connected);
...
static void update_gix_status(uint32_t interval_ms);
...
enum ControlTableItemAddr{
  ...
  // gix
  ADDR_CONNECT_GIX              = 346,
  ADDR_TORQUE_GIX               = 347,

  ADDR_GOAL_POSITION_GIX        = 348,
  ADDR_GOAL_POSITION_WR_GIX     = 352,
  ADDR_GOAL_POSITION_RD_GIX     = 353,

  ADDR_PRESENT_POSITION_GIX     = 355,
  ADDR_PRESENT_VELOCITY_GIX     = 359,
  ADDR_PRESENT_CURRENT_GIX      = 363,

  ADDR_PROFILE_ACC_GIX          = 365,
  ADDR_PROFILE_ACC_WR_GIX       = 369,
  ADDR_PROFILE_ACC_RD_GIX       = 370,

  ADDR_PROFILE_VEL_GIX          = 372,
  ADDR_PROFILE_VEL_WR_GIX       = 376,
  ADDR_PROFILE_VEL_RD_GIX       = 377,

  ADDR_GOAL_CURRENT_GIX         = 379,
  ADDR_GOAL_CURRENT_WR_GIX      = 381,
  ADDR_GOAL_CURRENT_RD_GIX      = 382,
};
...
typedef struct ControlItemVariables{
  ...
  // gix
  bool is_connect_gix_motor;

  bool gix_torque_enable_state;
  gix_position_info_t gix_goal_position;
  gix_position_info_t gix_present_position;
  gix_velocity_info_t gix_present_velocity;
  gix_current_info_t gix_present_current;
  gix_accel_info_t gix_profile_acc;
  gix_accel_info_t gix_profile_vel;
  gix_current_info_t gix_goal_current;

  bool gix_goal_position_wr;
  bool gix_goal_position_rd;

  bool gix_profile_acc_wr;
  bool gix_profile_acc_rd;

  bool gix_profile_vel_wr;
  bool gix_profile_vel_rd;

  bool gix_goal_current_wr;
  bool gix_goal_current_rd;
...
void TurtleBot3Core::begin(const char* model_name)
{
  ...
  if(strcmp(model_name, "Burger") == 0 || strcmp(model_name, "burger") == 0){
    ...
    model_motor_rpm = 77;
  } else if(strcmp(model_name, "GIX_Burger") == 0) {
    p_tb3_model_info = &gix_burger_info;
    model_motor_rpm = 61;
  } else if(strcmp(model_name, "GIX_Waffle") == 0) {
    p_tb3_model_info = &gix_waffle_info;
    model_motor_rpm = 77;
  }else{
    p_tb3_model_info = &burger_info;
    model_motor_rpm = 61;
  ...
  if (p_tb3_model_info->has_gix == true)
  {    
    ret = gix_motor_driver.init();
    DEBUG_PRINTLN(ret==true?"GIX driver setup completed.":"GIX driver setup failed.");
  }
  ...
  control_items.is_connect_gix_motor = false;

  // Port begin
  dxl_slave.begin();
  // Init DXL Slave functioC
  ...
  // gix
  dxl_slave.addControlItem(ADDR_CONNECT_GIX, control_items.is_connect_gix_motor);
  ...
  if (p_tb3_model_info->has_gix == true) {
    control_items.gix_goal_position_wr = false;
    control_items.gix_goal_position_rd = false;
    control_items.gix_profile_acc_wr = false;
    control_items.gix_profile_acc_rd = false;
    control_items.gix_profile_vel_wr = false;
    control_items.gix_profile_vel_rd = false;
    control_items.gix_goal_current_wr = false;
    control_items.gix_goal_current_rd = false;

    dxl_slave.addControlItem(ADDR_TORQUE_GIX, control_items.gix_torque_enable_state);

    dxl_slave.addControlItem(ADDR_GOAL_POSITION_GIX, control_items.gix_goal_position.value[GIX]);
    dxl_slave.addControlItem(ADDR_GOAL_POSITION_WR_GIX, control_items.gix_goal_position_wr);
    dxl_slave.addControlItem(ADDR_GOAL_POSITION_RD_GIX, control_items.gix_goal_position_rd);

    dxl_slave.addControlItem(ADDR_PRESENT_POSITION_GIX, control_items.gix_present_position.value[GIX]);

    dxl_slave.addControlItem(ADDR_PRESENT_VELOCITY_GIX, control_items.gix_present_velocity.value[GIX]);

    dxl_slave.addControlItem(ADDR_PRESENT_CURRENT_GIX, control_items.gix_present_current.value[GIX]);

    dxl_slave.addControlItem(ADDR_PROFILE_ACC_GIX, control_items.gix_profile_acc.value[GIX]);
    dxl_slave.addControlItem(ADDR_PROFILE_ACC_WR_GIX, control_items.gix_profile_acc_wr);
    dxl_slave.addControlItem(ADDR_PROFILE_ACC_RD_GIX, control_items.gix_profile_acc_rd);

    dxl_slave.addControlItem(ADDR_PROFILE_VEL_GIX, control_items.gix_profile_vel.value[GIX]);
    dxl_slave.addControlItem(ADDR_PROFILE_VEL_WR_GIX, control_items.gix_profile_vel_wr);
    dxl_slave.addControlItem(ADDR_PROFILE_VEL_RD_GIX, control_items.gix_profile_vel_rd);

    dxl_slave.addControlItem(ADDR_GOAL_CURRENT_GIX, control_items.gix_goal_current.value[GIX]);
    dxl_slave.addControlItem(ADDR_GOAL_CURRENT_WR_GIX, control_items.gix_goal_current_wr);
    dxl_slave.addControlItem(ADDR_GOAL_CURRENT_RD_GIX, control_items.gix_goal_current_rd);    
  }
  ...
  if (p_tb3_model_info->has_gix == true) {
    // Check connection state with gix motor.
    if(gix_motor_driver.is_connected() == true){
      gix_motor_driver.set_torque(true);    
      control_items.is_connect_gix_motor = true;
      set_connection_state_with_gix(true);
      DEBUG_PRINTLN("GIX motor is connected");      
    }else{
      control_items.is_connect_gix_motor = false;
      set_connection_state_with_gix(false);
      DEBUG_PRINTLN("Can't communicate with the GIX joint!");
      DEBUG_PRINTLN("  Please check the connection to the GIX motor and the power supply.");
      DEBUG_PRINTLN();
    } 
  }
  // Init IMU 
...
void TurtleBot3Core::run()
{
  ...
  update_gix_status(INTERVAL_MS_TO_UPDATE_CONTROL_ITEM);
...
void update_gix_status(uint32_t interval_ms)
{
  static uint32_t pre_time;
  ...
  if(millis() - pre_time >= interval_ms){
    ...
    gix_motor_driver.read_present_position(control_items.gix_present_position);
    gix_motor_driver.read_present_velocity(control_items.gix_present_velocity);
    gix_motor_driver.read_present_current(control_items.gix_present_current);

    if(get_connection_state_with_gix() == true){

      control_items.gix_torque_enable_state = gix_motor_driver.get_torque();
    }
  }  
}
...
static void dxl_slave_write_callback_func(uint16_t item_addr, uint8_t &dxl_err_code, void* arg)
{
  ...
  switch(item_addr)
  {
    ...
    // GIX TORQUE
    //
    case ADDR_TORQUE_GIX:
      gix_motor_driver.set_torque(control_items.gix_torque_enable_state);
      break;
    
    // ADDR_GOAL_POSITION_GIX
    //
    case ADDR_GOAL_POSITION_WR_GIX:
      if (get_connection_state_with_ros2_node() == true && control_items.gix_goal_position_wr == true) {
        gix_motor_driver.write_goal_position_joint(control_items.gix_goal_position);
      }
      control_items.gix_goal_position_wr = false;
      break;

    case ADDR_GOAL_POSITION_RD_GIX:
      if (control_items.gix_goal_position_rd == true) {
        gix_motor_driver.read_goal_position(control_items.gix_goal_position);
      }
      control_items.gix_goal_position_rd = false;
      break;

    // ADDR_PROFILE_ACC_GIX
    //
    case ADDR_PROFILE_ACC_WR_GIX:
      if (get_connection_state_with_ros2_node() == true && control_items.gix_profile_acc_wr == true) {
        gix_motor_driver.write_profile_acceleration_joint(control_items.gix_profile_acc);
      }
      control_items.gix_profile_acc_wr = false;
      break;      

    case ADDR_PROFILE_ACC_RD_GIX:
      if (control_items.gix_profile_acc_rd == true) {
        gix_motor_driver.read_profile_acceleration(control_items.gix_profile_acc);
      }
      control_items.gix_profile_acc_rd = false;
      break;     

    // ADDR_PROFILE_VEL_GIX
    //
    case ADDR_PROFILE_VEL_WR_GIX:
      if (get_connection_state_with_ros2_node() == true && control_items.gix_profile_vel_wr == true) {
        gix_motor_driver.write_profile_velocity_joint(control_items.gix_profile_vel);
      }
      control_items.gix_profile_vel_wr = false;
      break;      

    case ADDR_PROFILE_VEL_RD_GIX:
      if (control_items.gix_profile_vel_rd == true) {
        gix_motor_driver.read_profile_velocity(control_items.gix_profile_vel);
      }
      control_items.gix_profile_vel_rd = false;
      break;      

    // ADDR_GOAL_CURRENT_GIX
    //
    case ADDR_GOAL_CURRENT_WR_GIX:
      if (get_connection_state_with_ros2_node() == true && control_items.gix_goal_current_wr == true) {
        gix_motor_driver.write_goal_current_joint(control_items.gix_goal_current);
      }
      control_items.gix_goal_current_wr = false;
      break;      

    case ADDR_GOAL_CURRENT_RD_GIX:
      if (control_items.gix_goal_current_rd == true) {
        gix_motor_driver.read_goal_current(control_items.gix_goal_current);
      }
      control_items.gix_goal_current_rd = false;
      break;        
  }
...
/*******************************************************************************
* Function definition to check the connection with the gix motor.
*******************************************************************************/
static bool is_connected_gix = false;

static bool get_connection_state_with_gix()
{
  return is_connected_gix;
}

static void set_connection_state_with_gix(bool is_connected)
{
  is_connected_gix = is_connected;
}
